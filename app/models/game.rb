...
class Game < ActiveRecord::Base
  ...
  def play(b1 = nil, b2 = nil, b3 = nil)
    ...
    ball3 = b3 if b3.present? && frames.size == 9
    sum = ball1 + (ball2.present? ? ball2 : 0) + (ball3.present? ? ball3 : 0)
    frame =
      {
        ball_1: ball1,
        ball_2: ball2,
  ...
  def clac_score(frame_num)
    ...
    frames[frame_num][:score] += frames[frame_num - 1][:score]
    if current_index >= 2
